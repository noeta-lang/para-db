...
- `query.noe` / `schema.noe` / `repo.noe` / `sql.noe` / `reactive.noe` — the pure-Noeta layers over the native driver. `schema.noe` renders the portable schema DSL; the grammar and the lowering are native, so the builder and a `.schema` migration file share one implementation.
