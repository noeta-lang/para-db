...
- A sub-directory of `migrations/` (or `seeds/`) means exactly one thing: a **per-dialect override** directory named by `schema::Dialect::dir_name` (`sqlite/`, `postgres/`), whose files replace the body of the same-named file above them for the connected driver. An override with no base file is an error, and so is any other sub-directory holding body files. `dir_name` is a dialect's single spelling, so a new backend adds its variant there — and `SqlDriver::dialect()` must report the dialect that driver's `lower_schema` renders in, since one selects the override the other lowers beside.
